...
mydata_wide <- mydata_wide %>% filter(SWWRPANAME_NEW != "Macquarie-Castlereagh") %>%
  filter(SWWRPANAME_NEW != "New South Wales Murray") #%>%
  #filter(SWWRPANAME_NEW != "Murrumbidgee")



b2 <- brm(Golden ~ Method + SWWRPANAME_NEW+ s(days_since, by = SWWRPANAME_NEW) +
            (1 + Method | SiteID) + (1 + Method | fDate)+
          ...
          file_refit = "always",
          seed=1234567,
          file = paste0(taxa[j],"updated_newcomp_boat_WRPA.rds")) # "../../srv/scratch/z3374139/MDB models/",
...
yy <- f2 %>% epred_draws(newdata = expand_grid(Method = "BTE",
                         ...
                         re_formula = NA) %>%
  mutate(Date = days_since + min(mydata_wide$Date), Species = "Macquarie perch")

write_csv(yy, "Macquarie perch updated good prediction plot data WRPA.csv")
...
ggsave("Macquarie perch updated good prediction plot WRPA.pdf", width = 21, height=23, units="cm", dpi =600)
ggsave("Macquarie perch updated good prediction plot WRPA.png", width = 21, height=23, units="cm", dpi =600)
