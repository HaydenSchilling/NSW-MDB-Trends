...
write_csv(yy, "Common carp BIOMASS updated good prediction plot data WRPA.csv")
...
ggsave("Common carp BIOMASS updated good prediction plot WRPA.pdf", width = 21, height=23, units="cm", dpi =600)
ggsave("Common carp BIOMASS updated good prediction plot WRPA.png", width = 21, height=23, units="cm", dpi =600)

mydata_wide <- mydata_wide %>% mutate(Year = as.integer(lubridate::year(Date))) %>% ungroup()
ggplot(mydata_wide, aes(Year, sqrt(Golden+0.001), group =Year)) + geom_boxplot() + facet_wrap(~SWWRPANAME_NEW, scales = "free_y")
