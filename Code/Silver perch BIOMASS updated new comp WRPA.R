...
mydata_wide <- mydata_wide %>% filter(SWWRPANAME_NEW != "Barwon-Darling Watercourse") %>%
  filter(SWWRPANAME_NEW != "Macquarie-Castlereagh") %>%
  filter(SWWRPANAME_NEW != "Namoi")

### Add biomass weights
biom <- read_csv("Silver perch operation mean weights.csv") %>%
...
library(brms)
b2 <- brm(Biomass ~ SWWRPANAME_NEW + s(days_since, by = SWWRPANAME_NEW) + # s(Month, bs="cc")+
            (1|SiteID) + (1|fDate)+
          ...
          file_refit = "always",
          seed=123456789,
          file = paste0(taxa[j],"_updated_BIOMASS_newcomp_boat_WRPA.rds")) # "../../srv/scratch/z3374139/MDB models/",
# 
# summary(b2)
...
yy <- b2 %>% epred_draws(newdata = expand_grid(Method = "BTE",
                         ...
                         re_formula = NA) %>%
  mutate(Date = days_since + min(mydata_wide$Date), Species = "Silver perch")

write_csv(yy, "Silver perch BIOMASS updated good prediction plot data WRPA.csv")
...
ggsave("Silver perch BIOMASS good prediction plot WRPA.pdf", width = 21, height=14.8, units="cm", dpi =600)
ggsave("Silver perch BIOMASS good prediction plot WRPA.png", width = 21, height=14.8, units="cm", dpi =600)