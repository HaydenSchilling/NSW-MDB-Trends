...
### Add biomass weights
biom <- read_csv("Murray cod operation mean weights.csv") %>%
...
yy <- b2 %>% epred_draws(newdata = expand_grid(Method = "BTE",
                         ...
                         re_formula = NA) %>%
  mutate(Date = days_since + min(mydata_wide$Date), Species = "Murray cod")

write_csv(yy, "Murray cod BIOMASS updated good prediction plot data WRPA.csv")
...
ggsave("Murray cod BIOMASS good prediction plot WRPA.pdf", width = 21, height=14.8, units="cm", dpi =600)
ggsave("Murray cod BIOMASS good prediction plot WRPA.png", width = 21, height=14.8, units="cm", dpi =600)