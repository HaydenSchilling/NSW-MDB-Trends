...
### Add biomass weights
biom <- read_csv("Golden perch operation mean weights.csv") %>%
...
model.check_f5 <- createDHARMa(
  simulatedResponse = t(posterior_predict(b2)),
  observedResponse = mydata_wide$Golden,
  fittedPredictedResponse = apply(t(posterior_epred(b2)), 1, mean),
  integerResponse = T,
  method = "traditional")
...
write_csv(yy, "Golden perch BIOMASS updated good prediction plot data WRPA.csv")
...
ggsave("Golden perch BIOMASS good prediction plot WRPA.pdf", width = 21, height=14.8, units="cm", dpi =600)
ggsave("Golden perch BIOMASS good prediction plot WRPA.png", width = 21, height=14.8, units="cm", dpi =600)